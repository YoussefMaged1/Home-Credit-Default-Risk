...
def evaluate_models(cfg: DictConfig, logger):
    ...
    try:
        ...
        logger.error("OOF files not found! Make sure to save y_train, xgb_oof, and cbm_oof in train.py")
        return

    ensemble_oof_preds = (0.3 * xgb_oof) + (0.7 * cbm_oof)
    ...
    logger.info(f"Final Ensemble AUC (70/30): {ensemble_auc:.5f}")
    ...
    try:
        ...
        df_sample = pd.read_csv(os.path.join(SOURCE, 'train.csv')).sample(2000, random_state=42)
        ...
        def ensemble_predict_fixed(X):
            ...
            return (0.3 * x_probs) + (0.7 * c_probs)
        ...
        sns.barplot(data=perm_importance_df.head(15), x='Importance', y='Feature', hue='Feature', legend=False)       
    ...
    final_summary = {
        ...
        "threshold": threshold,
        "classification_report": classification_report(y_train, y_pred_binary, output_dict=True)
    }
