...
def get_bureau_features(bureau: pd.DataFrame):
...
def get_prev_apps_features(prev: pd.DataFrame):
...
def get_installments_features(installments: pd.DataFrame):
    ins = installments.copy()

    ins["PAYMENT_DIFF"] = ins["AMT_INSTALMENT"] - ins["AMT_PAYMENT"]

    ins["DPD"] = ins["DAYS_ENTRY_PAYMENT"] - ins["DAYS_INSTALMENT"]
    ins["DPD"] = ins["DPD"].clip(lower=0).fillna(0)
...
def read_process_data(config, logger) -> None:
    ...
    X_test.to_csv(os.path.join(DESTINATION, "test.csv"), index=False)
